# 📚 Engineering: Programming and Electronics

This repository has within in it the course materials for a course in 


---

## Units

### Unit 1: Electricity, Python, and Git (5 days)

- **Day 1:** Intall GitHub Desktop, Thonny, introduce Git
- **Day 2:** Continue with branches, commits, push, pull, pull requests. Intro Ohm's Law and electricity
- **Day 3:** Intro to Electrical circuits. Make circuit without programming
- **Day 4:** Upload MicroPython and start programming digital outputs
- **Day 5:** Start project to create their own circuit and behaviors
- **Day 6:** Finish Project


### Unit 2: Control and Input Making interactive Circuits (3 days)

- **Day 1:** 
- **Day 2:**
- **Day 3:**


### Unit 3: Motors, Servos, and Power (4 days)

- **Day 1:** 
- **Day 2:**
- **Day 3:**
- **Day 4:**

### Unit 4: Design Project 1 (5 days)

- **Day 1:**
- **Day 2:**
- **Day 3:**
- **Day 4:**
- **Day 5:**

### Unit 5: Data, Classes, and System Thinking (5 days)

- **Day 1:**
- **Day 2:**
- **Day 3:**
- **Day 4:**
- **Day 5:**

### Unit 6: Final Design Challenge (5 days)

- **Day 1:**
- **Day 2:**
- **Day 3:**
- **Day 4:**
- **Day 5:**

**Current Plan is 27 days (5 still to be determined)**

## Day by Day Term 2

### Week 1 

* **Day 1**: Intall GitHub Desktop, Thonny, introduce Git
* **Day 2**: Continue with branches, commits, push, pull, pull requests. Intro Ohm's Law and electricity
* **Day 3**: Intro to Electrical circuits. Make circuit without programming

### Week 2

* **Day 4**: Upload MicroPython and start programming digital outputs
* **Day 5**: Start project to create their own circuit and behaviors
* **Day 6**: Finish Project

**THANKSGIVING BREAK**

### Week 3

*Start Unit 2*

* **Day 6**: Wiring push buttons, pull up and push down resistors. If-else statements
* **Day 7**: Quiz, Potentiometers, Analog vs. digital

### Week 4

*Finish Unit 2, Start Unit 3*

* **Day 8**: Mini-project to incorporate outputs and inputs 
* **Day 9**: Test 
* **Day 10**: Servos and libraries. Connect servo and potentiometer

### Week 5

* **Day 11**: Transistors and DC motors. Multiple voltages. 
* **Day 12**: Motor controllers
* **Day 13**: quadrature encoders

**WINTER BREAK**

### Week 6

*Unit 4 Starts*

* **Day 14**: Introduction to wiring diagrams and simulators (if we haven't done that yet)
* **Day 15**: Idea generation and sharing
* **Day 16**: Build Day

### Week 7

* **Day 17**: Report out and get feedback
* **Day 18**: Build Day
* **Day 19**: Final presentations



### Week 8

*Unit 5 starts*

* **Day 20**: Introduction to OOP 
* **Day 21**: Construction of a Dataclass

### Week 9

* **Day 22**: Connecting saving data found with Pico
* **Day 23**: Connecting Pico to Wifi
* **Day 24**: Test

### Week 10

*Unit 6 Starts*

* **Day 25**: 
* **Day 26**:
* **Day 27**:


### Week 11

* **Day 28**: 
* **Day 29**:

### Week 12

* **Day 30**: 
* **Day 31**:
